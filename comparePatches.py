...
def getOnePatchFeatureSetScore(this_feature_set, this_feature_weights, patch):
	...
	patch: patch with computed feature scores
	return: the weighted feature set score for this patch
	"""
	one_response = 0.0
	for i in range(0, len(this_feature_set)):
		cur_feature_obj = patch.getFeatureObject(this_feature_set[i])
		cur_feature_weight = this_feature_weights[i]
		one_response += cur_feature_obj.score * cur_feature_weight
	return one_response

def LDAFeatureScore(this_feature_set, this_feature_weights, testPatch, random_patches_response, plotHist = False,  path = "", testPatchIndex = 0):
	"""
	this_feature_set: feature sets to consider
	this_feature_weights: weights of features in the set
...
def main():
	...
	global FEATURES
	FEATURES = copy.deepcopy(utils.ALL_FEATURE_IDS)
	image_db = raw_input("please input image_db name(from root folder):")
	test_folder_name = raw_input("please input the test_folder_name:")
	test_img_name = raw_input("please input the test_img_name (with extension, eg: test1.jpg):")
	sigma = compute_sigma(cv2.imread("{image_db}/{folder}/{image}".format(\
		image_db = image_db, folder = test_folder_name, image = test_img_name)))
	"""Test Algo3 in finding distinguishable patches"""
	populateTestFindDistinguishablePatchesAlgo3(test_folder_name, test_img_name, sigma, custom_features_name = "ALL_FEATURE_IDS")
	return
